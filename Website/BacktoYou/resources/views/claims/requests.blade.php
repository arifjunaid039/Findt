...
@media (max-width:991.98px){
    .wrap{ width:94%; }
    .claim-card{ padding:22px; gap:18px; }
}

/* ---------- tablet ---------- */
@media (max-width: 767.98px){
    .wrap{ width:95%; margin:36px auto; }
    .claim-card{ padding:20px; gap:16px; }
    .filters{ max-width:100%; }
}

/* ---------- phone ---------- */
@media(max-width:576px){
    .wrap{ width:100%; padding:0 16px; margin:24px auto; }
    .heading{ font-size:24px; }
    .subtitle{ font-size:13px; margin-bottom:18px; }

    .claim-card{ flex-direction:column; padding:16px; border-radius:16px; }
    .item-photo, .item-placeholder{ width:100%; height:140px; }

    .top-row{ gap:6px; flex-direction:column; align-items:flex-start; }
    .item-title{ font-size:17px; }
    .claim-date{ align-self:flex-start; }

    .user{ font-size:13.5px; }

    .message{ padding:12px 14px; font-size:14px; margin:12px 0; }

    /* Stack Approve/Reject full-width so they're easy to tap and don't
       compete for space next to each other on a narrow screen. */
    ...
    .filters{ width:100%; }
    .filter-btn{ flex:1 0 auto; text-align:center; padding:8px 10px; }
}

/* ---------- small phone ---------- */
@media(max-width:400px){
    .wrap{ padding:0 12px; }
    .heading{ font-size:21px; gap:8px; }
    .claim-card{ padding:14px; border-radius:14px; }
    .item-photo, .item-placeholder{ height:110px; }
    .status{ font-size:12px; padding:5px 12px; }
    .btn-approve, .btn-reject{ padding:10px 16px; font-size:13.5px; }
}

/* ---------- extra small phone ---------- */
@media(max-width:340px){
    .heading{ font-size:19px; }
    .filter-btn{ font-size:12px; padding:7px 8px; }
...
<div class="wrap" x-data="claims()">
    ...
    @forelse($claims as $claim)
        ...
        <div class="claim-card" data-status="{{ $claim->status }}">
            ...
            @if(isset($claim->photo) && $claim->photo)
                <img src="{{ asset('uploads/items/'.$claim->photo) }}" class="item-photo" alt="{{ $claim->title }}">
            @else
