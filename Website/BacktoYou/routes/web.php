...
/*
|--------------------------------------------------------------------------
| AUTHENTICATED USER ROUTES (phone verification required)
|--------------------------------------------------------------------------
| Every route in this group requires the user to be logged in AND to
| have a verified phone number. Unverified users get bounced to
| /verify-phone automatically by the EnsurePhoneVerified middleware.
*/

Route::middleware(['auth', 'phone.verified', \App\Http\Middleware\NoCacheHeaders::class])->group(function () {
    ...
    */
    ...
    Route::get('/Items', function () {
        ...
        $location = request('location'); // NEW: selected location filter
        ...
        if ($location) {
            $query->where('items.location', $location); // NEW: filter by exact location
        }
        ...
        if ($communityId) {
            ...
            if ($selectedCommunity) {
                $query->where('items.community_id', $communityId);
            }
        } elseif (Auth::check()) {
            $myCommunityIds = Auth::user()->communities()->pluck('communities.id')->toArray();

            if (count($myCommunityIds)) {
                $query->orderByRaw(
                    "FIELD(items.community_id, " . implode(',', array_fill(0, count($myCommunityIds), '?')) . ") DESC",
                    $myCommunityIds
                );
            }
        }
        ...
        // NEW: distinct locations across ALL items, used to build the location filter chips
        ...
        return view('Items', compact('items', 'categories', 'type', 'selectedCommunity', 'location', 'locations'));
    })->name('items.all');
...
| LOGOUT
|--------------------------------------------------------------------------
| Deliberately kept OUTSIDE the phone.verified group — an unverified
| user still needs to be able to log out of their account.
*/
...
Route::middleware('auth')->group(function () {
    Route::get('/verify-phone', [PhoneVerificationController::class, 'show'])->name('verify-phone.show');
    Route::post('/verify-phone/send', [PhoneVerificationController::class, 'send'])->name('verify-phone.send');
    Route::post('/verify-phone/verify', [PhoneVerificationController::class, 'verify'])->name('verify-phone.verify');
