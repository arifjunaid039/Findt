...
<html>
<head>
    <title> BackToYou | Manage Profile</title>
...
/* MY COMMUNITIES — blue/white theme */
.communities-section{
    margin-top:26px;
}

/* The whole header row is a button that toggles the dropdown,
   styled as a soft blue gradient bar rather than a plain text row */
.section-title-btn{
    width:100%;
...
.section-title-btn:hover{
    border-color:#bfdbfe;
    background:linear-gradient(135deg,#e6efff,#dbe4ff);
}

.section-title-btn[aria-expanded="true"]{
...
.section-title-btn[aria-expanded="true"] .section-chevron{
    transform:rotate(180deg);
}

/* Collapsible wrapper — sits flush under the blue header like a real
   dropdown panel, with its own white card body and blue outline */
.community-dropdown{
    max-height:0;
    overflow:hidden;
...
/* Dark mode — keep the same blue identity, just deepened */
body.dark .section-title-btn{
    background:linear-gradient(135deg,rgba(37,99,235,.16),rgba(37,99,235,.1));
    border-color:rgba(59,130,246,.35);
}

body.dark .section-title-btn:hover{
    background:linear-gradient(135deg,rgba(37,99,235,.22),rgba(37,99,235,.14));
...
body.dark .section-chevron{
    color:#93c5fd;
}

body.dark .community-dropdown{
    background:#0f172a;
    border-color:rgba(59,130,246,.35);
}
...
<div class="container">
    ...
    <div class="communities-section">
        ...
        <button type="button" class="section-title-btn" id="communitiesToggle" aria-expanded="false" aria-controls="communitiesDropdown">
            <h3 class="section-title">
                <i class="fa-solid fa-users"></i> My Communities
                <span class="community-count-badge">{{ $myCommunities->count() }}</span>
            </h3>
            <i class="fa-solid fa-chevron-down section-chevron"></i>
        </button>

        <div class="community-dropdown" id="communitiesDropdown">
...
<!-- Isolated in its own <script> tag on purpose: if the photo-upload
     script above throws for any reason, it stops executing the rest
     of ITS block, but this separate tag still runs regardless. -->
<script>
(function(){
    const toggleBtn = document.getElementById('communitiesToggle');
